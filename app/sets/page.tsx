...
const SetsPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const mappedSets: Set[] = await fetchSets();
            setSets(mappedSets);
    ...
    useEffect(() => {
        ...
        const sorted = filteredSets.sort((set1, set2) => {
            let comparison = 0;
            if (sortOption === "name") {
            comparison = set1.name.localeCompare(set2.name);
            } else {
            comparison =
                new Date(set1.releaseDate).getTime() -
                new Date(set2.releaseDate).getTime();
            }
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-8 space-y-4 md:space-y-0">
            ...
            <div className="relative" ref={dropdownRef}>
                ...
                <div className="absolute right-0 mt-2 w-48 bg-gray-800 border border-gray-700 rounded-md shadow-lg z-10">
                    ...
                    <p className="font-bold text-muted-foreground">Set Types</p>
                    {tags.map((tag, index) => (
